...
import { X, CheckCircle, AlertCircle, Building2, MapPin, Clock,
  BadgeDollarSign, Zap, Target, Sparkles, Send, Briefcase, FileText } from "lucide-react";
...
const ApplyModal = ({ isOpen, onClose, internship }) => {
  ...
  const handleApply = async () => {
    setLoading(true);
    setMessage({ type: "", text: "", quote: "" });
    try {
      await api.post("/applications", { internshipId: internship._id });
      const randomQuote = MOTIVATIONAL_QUOTES[Math.floor(Math.random() * MOTIVATIONAL_QUOTES.length)];
      setMessage({ type: "success", text: "Application submitted successfully!", quote: randomQuote });
      setTimeout(() => {
        onClose();
        window.location.reload();
      }, 6000);
    } catch (err) {
      const errorText = err.response?.data?.message || "Failed to apply";
      if (errorText.toLowerCase().includes("already applied")) {
        const randomQuote = MOTIVATIONAL_QUOTES[Math.floor(Math.random() * MOTIVATIONAL_QUOTES.length)];
        setMessage({ type: "warning", text: errorText, quote: randomQuote });
        setTimeout(() => {
          onClose();
          window.location.reload();
        }, 6000);
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className={`relative bg-white rounded-2xl shadow-2xl w-full ${['success', 'warning'].includes(message.type) ? "max-w-sm p-8" : "max-w-md max-h-[92vh]"} overflow-hidden flex flex-col animate-fade-up`}>
        {['success', 'warning'].includes(message.type) ? (
          <div className="text-center flex flex-col items-center animate-fade-in">
            {message.type === "success" ? (
              ...
              <div className="w-16 h-16 bg-amber-100 rounded-full flex items-center justify-center mb-5 shadow-inner">
                <AlertCircle className="w-8 h-8 text-amber-600" />
              </div>
            )}
            <h3 className="text-xl font-bold text-gray-900 leading-tight">{message.text}</h3>
            {message.quote && (
              <p className={`italic mt-3 font-medium text-sm ${message.type === 'success' ? 'text-emerald-700' : 'text-amber-700'}`}>
                "{message.quote}"
              </p>
            )}
          ...
          <>
            ...
            <div className="overflow-y-auto p-5 pt-10 space-y-4">
              ...
              {/* Message Alert (Only for errors now) */}
              {message.text && message.type === "error" && (
                <div className="flex items-start gap-3 p-4 rounded-xl text-sm shadow-sm bg-red-50 text-red-800 border border-red-200">
