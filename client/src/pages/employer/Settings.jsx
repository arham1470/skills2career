...
const Settings = () => {
  ...
  const handleDeleteAccount = async (e) => {
    ...
    setDeleting(true);
    try {
      await api.delete("/provider/account", { data: { currentPassword: deletePassword } });
      logout();
      navigate("/");
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await api.put("/provider/password", { currentPassword: form.currentPassword, newPassword: form.newPassword });
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 border-t-4 border-t-red-500 p-6 shadow-sm">
        ...
        <p className="text-sm text-red-600/80 mb-4">
          Deleting your account will permanently remove all your data, including company profile, internships, applications, and messages. This action cannot be undone.
        </p>
        <Button
          variant="secondary"
          onClick={() => { setShowDeleteDialog(true); setDeletePassword(""); setDeleteError(""); }}
          className="bg-red-600 text-white border-transparent hover:bg-red-700 focus:ring-red-500 shadow-sm"
        >
